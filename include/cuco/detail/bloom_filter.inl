...
bloom_filter<Key, Scope, Allocator, Slot>::bloom_filter(std::size_t num_bits,
                                                        ...
                                                        cudaStream_t stream)
    : num_bits_{SDIV(std::max(std::size_t{1}, num_bits),
                     detail::type_bits<slot_type>()) *
                detail::type_bits<slot_type>()},
      num_slots_{SDIV(std::max(std::size_t{1}, num_bits),
                      detail::type_bits<slot_type>())},
      num_hashes_{std::clamp(num_hashes, std::size_t{1},
                             detail::type_bits<slot_type>())},
      slot_allocator_{alloc} {
  slots_ = std::allocator_traits<slot_allocator_type>::allocate(slot_allocator_,
                                                                num_slots_);
...
bloom_filter<Key, Scope, Allocator, Slot>::~bloom_filter() {
  std::allocator_traits<slot_allocator_type>::deallocate(slot_allocator_,
                                                         slots_, num_slots_);
}

template <typename Key, cuda::thread_scope Scope, typename Allocator,
          typename Slot>
void bloom_filter<Key, Scope, Allocator, Slot>::initialize(
    cudaStream_t stream) {
  ...
  detail::initialize<block_size>
      <<<grid_size, block_size, 0, stream>>>(slots_, num_slots_);
}
...
template <typename Key, cuda::thread_scope Scope, typename Allocator,
          typename Slot>
template <typename InputIt, typename StencilIt, typename Predicate, typename Hash1, typename Hash2, typename Hash3>
void bloom_filter<Key, Scope, Allocator, Slot>::insert_if(InputIt first,
  ...
  detail::insert_if<block_size><<<grid_size, block_size, 0, stream>>>(
      first, last, get_device_mutable_view(), stencil, pred, hash1, hash2, hash3);
}
...
void bloom_filter<Key, Scope, Allocator, Slot>::contains(
    InputIt first, InputIt last, OutputIt output_begin, cudaStream_t stream,
...
__device__ Slot
bloom_filter<Key, Scope, Allocator, Slot>::device_view_base::key_pattern(
  ...
  while (k < num_hashes_) {
    ...
    slot_type const bit = slot_type{1}
                          << ((h1 + (i * h2) + ((i * i * i - i) / 6)) %
                              detail::type_bits<slot_type>());
...
__device__ bool
bloom_filter<Key, Scope, Allocator, Slot>::device_mutable_view::insert(
...
template <typename Hash1, typename Hash2, typename Hash3>
__device__ bool
bloom_filter<Key, Scope, Allocator, Slot>::device_view::contains(
