...
int main(void) {
  ...
  // Insert all keys and subsequently query them against the filter; measure
  // runtime
  ...
  cuco::register_l2_persistence(stream, filter.get_slots(),
                                filter.get_slots() + filter.get_num_slots());
